...
function syncActiveSlide() {
  document.querySelectorAll(".slide").forEach((slide, index) => {
    slide.classList.toggle("is-active", index === cur);
  });
}
...
function handleSearch(q) {
  ...
  renderSearchResults(
    filtered.length
      ? filtered
      : [{ icon: "❌", text: 'No results found for "' + q + '"' }],
  );
}
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    if (document.getElementById("searchOverlay").classList.contains("open"))
      toggleSearch();
    if (document.getElementById("aiWindow").classList.contains("open"))
      toggleAI();
...
const aiKnowledge = {
  ...
  programmes:
    "DPS Ghana offers: 🌱 Cambridge Pre-Primary (3–5 yrs) | 📚 Cambridge Primary (5–11 yrs) | 🔬 Cambridge Lower Secondary (11–14 yrs) | 🎓 Cambridge IGCSE (14–16 yrs) | ⭐ AS & A Level (16–18 yrs) | 🌍 IBDP (16–18 yrs).",
  location:
    "📍 DPS International Ghana is located at Community 25 (P2P7+C23), Tema, Greater Accra Region, Ghana. We are about 30–40 minutes from central Accra via the N1 motorway. School buses serve 19 routes across Tema and Accra.",
  transport:
    "DPSI operates 22 school buses across 19 different routes covering Tema and Accra. All buses are GPS-tracked, air-conditioned, and driven by 26 professional dedicated drivers. Around 200 students commute daily.",
  contact:
    "📞 Call us: +233 55 662 0540 | +233 54 435 3199 | +233 54 434 9579\n✉️ Email: Principal@dpsghana.edu.gh\n🕐 Office Hours: Mon–Fri 7AM–5PM | Sat 8AM–1PM",
  "pre-primary":
    "Our Cambridge Pre-Primary programme accepts children aged 3 to 5 years. It's a nurturing, play-based environment that develops curiosity, creativity and a love of learning through discovery.",
  igcse:
    "Cambridge IGCSE is offered for students aged 14–16. It's one of the world's most recognised qualifications, offered across 70+ subjects. Our students consistently achieve outstanding results with 94%+ scoring A*–C.",
  ...
  facilities:
    "DPS Ghana offers: 🏫 State-of-the-art infrastructure | 📺 Smart classrooms with digital boards | 🔬 Physics, Chemistry & Biology labs | 💻 2 computer labs | 📚 Well-stocked library | 🏊 Olympic-size swimming pool | ⚽ Football pitch | 🏀 Basketball courts | 🎭 1000-seat Amphitheatre | 🏛️ 700-seat Auditorium | 🍽️ School canteen | 🚌 School transport.",
  default:
    "I'm here to help with information about DPS International Ghana! You can ask me about admissions, programmes (IGCSE, A-Level, IBDP, Primary), fees, facilities, location, transport, or contact details. What would you like to know?",
};

function getAIResponse(msg) {
  const m = msg.toLowerCase();
  if (m.includes("admission") || m.includes("apply") || m.includes("enrol"))
    return aiKnowledge.admission;
  if (
    m.includes("fee") ||
    m.includes("cost") ||
    m.includes("price") ||
  ...
  if (
    ...
    return aiKnowledge.location;
  if (m.includes("transport") || m.includes("bus") || m.includes("route"))
    return aiKnowledge.transport;
  if (
    m.includes("contact") ||
    m.includes("phone") ||
    m.includes("email") ||
    ...
    return aiKnowledge.contact;
  if (
    m.includes("pre-primary") ||
    m.includes("creche") ||
    m.includes("nursery") ||
    m.includes("3") ||
    m.includes("4") ||
    ...
    return aiKnowledge["pre-primary"];
  if (m.includes("igcse") || m.includes("gcse")) return aiKnowledge.igcse;
  if (m.includes("ib") || m.includes("diploma") || m.includes("ibdp"))
    return aiKnowledge.ibdp;
  if (
    m.includes("facilit") ||
    m.includes("lab") ||
    m.includes("pool") ||
...
function submitEnroll() {
  ...
  const hasErrors = [phone, first, last, age].some(
    (f) => f.classList.contains("error") || !f.value.trim(),
  );
